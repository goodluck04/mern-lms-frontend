...
const CourseCard: FC<Props> = ({ item, isProfile }) => {
    return (
        <Link href={!isProfile ? `/course/${item._id}` : `course-access/${item._id}`}>
            <div className='w-full min-h-[35vh] dark:bg-opacity-20 dark:bg-slate-500 backdrop-blur border dark:border-[#ffffff1d] border-[#00000015] dark:shadow-[bg-slate-700] rounded-lg p-3 shadow-sm dark:shadow-inner'>
                <Image src={item.thumbnail.url} width={500} height={500} objectFit="contain" className='rounded w-full' alt='' />
                <br />
